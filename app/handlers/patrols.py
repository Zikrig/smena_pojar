...
async def handle_base1_selection(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "4. Отправьте фото"
        "5 Если в ходе обхода обнаружена проблема фото метки делать не нужно - жмите на кнопку \"Проблема\" и дальше описывайте ее в сообщении.",
...
async def process_patrol_photo(message: Message, state: FSMContext, patrol_type: str):
    ...
    if current_step < 5:
        ...
        await message.answer(
            ...
            "4. Отправьте фото"
            "5 Если в ходе обхода обнаружено нарушение - фото метки делать не нужно - жмите на кнопку \"Нарушение\" и дальше описывайте ее в сообщении. Также о нарушениях можно сообщать по кнопке ⚠️ Проблема",
